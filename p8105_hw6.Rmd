...
```{r problem_1_Baltimore, message = FALSE, warning = FALSE}
library(broom)
...
```{r problem_1_city, message = FALSE, warning = FALSE}
city_data =
  homicide_data_clean |>
  group_by(city_state) |>
  tidyr::nest() |>
...
## Problem3
```{r problem_3, message = FALSE, warning = FALSE}
birthweight = 
...
```{r problem_3_plot, message = FALSE, warning = FALSE}
library(modelr)
library(ggplot2)
...
```
```{r problem_3_cross_validation, message = FALSE, warning = FALSE}
library(modelr)
library(mgcv)
library(purrr)
...
cv_result
```
```{r problem_3_cv_plot, message = FALSE, warning = FALSE}
cv_result |> 
